...
class KNN:
    ...
    def compute_distances_no_loops(self, X):
        ...
        if self.metric == 'euclidean':
            # TODO: Implement computing all distances with no loops!
            square_sum_feature_train = np.sum(np.square(self.train_X), axis=1)
            square_sum_feature_test = np.sum(np.square(X), axis=1)
            dot_matrix = X.dot(self.train_X.transpose())
            dists = dists - (2*dot_matrix) + square_sum_feature_test[:, np.newaxis] + square_sum_feature_train
            dists = np.sqrt(dists)
            return dists
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            k_min_index = np.argsort(dists[i])[:self.k]
            pred[i] = np.argmax(np.bincount(self.train_y[k_min_index]))
        return pred
